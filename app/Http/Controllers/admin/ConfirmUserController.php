...
{    
    ...
    public function complete(Request $request){
        //get the bookings where status == 1 
        $confirmbookings = Booking::where('status', 1);
        $confirmbookings = $confirmbookings->paginate(10);
        return view('admin.users.complete', compact('confirmbookings'));
    }
    ...
    public function index(Request $request)
    {
        try {
            ...
            if (isset($request->status) && !empty($request->status)) {
                $draftUsers = $draftUsers->where('is_draft', $request->status);
